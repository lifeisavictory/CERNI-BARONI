...
- Entry point: `index.html` (contains SEO/OG meta tags) → `src/main.jsx` imports self-hosted font CSS (`@fontsource-variable/fraunces`, `@fontsource/inter`, `@fontsource/ibm-plex-mono`) then `index.css`, and wraps `<App />` in `<BrowserRouter>` and `<StrictMode>`, mounting into `#root`.
...
- **Styling**: `src/index.css` defines global design tokens as CSS custom properties on `:root` (`--text`, `--text-h`, `--bg`, `--page-bg`, `--border`, `--accent`, `--accent-bg`, `--accent-border`, `--shadow`, font stacks `--sans`/`--heading`/`--mono`), with a dark-theme override block under `@media (prefers-color-scheme: dark)`. Always reuse these tokens instead of hardcoding colors/fonts. `src/App.css` holds component/layout-scoped styles using native CSS nesting (targets modern browsers per Vite's default build — no PostCSS nesting plugin). Fonts: Fraunces (headings), Inter (body), IBM Plex Mono (nav/labels) — self-hosted (not Google Fonts CDN) via `@fontsource` npm packages, imported as CSS in `src/main.jsx` and bundled/hashed by Vite like any other asset. `--heading` resolves to the family name `'Fraunces Variable'` (as published by `@fontsource-variable/fraunces`), not plain `'Fraunces'`. Only the `latin`/`latin-ext` subset files and the weights/styles actually used in the CSS are imported (site content is Czech only, so `cyrillic`/`greek`/`vietnamese` subsets are deliberately excluded to keep `dist/` smaller): Inter `latin(-ext)-400`/`400-italic`/`600`/`700`, IBM Plex Mono `latin(-ext)-400`/`600`. `@fontsource-variable/fraunces` ships no pre-split latin-only CSS, so `src/fonts.css` hand-declares the 4 needed `@font-face` blocks (normal/italic × latin/latin-ext) referencing `@fontsource-variable/fraunces/files/*.woff2` directly via Vite's bare-specifier `url()` resolution. Check actual CSS usage before adding a new weight/subset import rather than assuming Google's original set.
